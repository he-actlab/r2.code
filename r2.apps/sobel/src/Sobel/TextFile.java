...
//ToDo: Complete loadObject for loadObjectArray.
/*
	public void loadObject (TextSerializable obj) throws IOException {
		if (openStatus && mode == Mode.READ) {
			obj.load (this);
		}
	}
...
//ToDo: Complete loadObjectArray.
/*
	public void loadObjectArray (TextSerializable[] objArray)
	    throws IOException, IllegalAccessException, InstantiationException {
		Object[] tempObjArray;
		if (openStatus && mode == Mode.READ) {
			Class c = objArray.getClass ();
			tempObjArray = (Object[])c.newInstance ();
			objArray =

			//(
			loadChar ();
			//length
			objArray = c.newInstance ();
			//,
			loadChar ();
			//[
			loadChar ();
	...
	//Template
	{
		...
		//length,
		parameterArray = new <Object>[inFile.loadInt ()];
		inFile.loadChar ();
		...
		for (int i = 0; i < parameterArray.length; i++) {
			parameterArray[i] = new <Object>;
			parameterArray[i].load (inFile);
			//, ]
			inFile.loadChar ();
		}
