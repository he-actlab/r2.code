...
public class FinderPatternFinder {
  ...
  FinderPatternInfo find(Hashtable hints) throws NotFoundException {
    boolean tryHarder = true; //hints != null && hints.containsKey(DecodeHintType.TRY_HARDER);
   ...
   */
  private static float centerFromEnd(int[] stateCount,
                                     int end) {
   ...
   */
  private float crossCheckVertical(int startI, int centerJ,
   ...
   */
  private float crossCheckHorizontal(int startJ,
      int centerI, int maxCount,
  ...
  protected boolean handlePossibleCenter(int[] stateCount, int i, int j) {
    ...
    if (!ApproxMath.isNaN(centerI)) {
      // Re-cross check
      centerJ = crossCheckHorizontal((int)centerJ, (int)centerI, stateCount[2], stateCountTotal);
      if (!ApproxMath.isNaN(centerJ)) {
  ...
  private boolean haveMultiplyConfirmedCenters() {
    ...
    for (int i = 0; i < max; i++) {
      FinderPattern pattern = (FinderPattern) possibleCenters.elementAt(i);
      totalDeviation += ApproxMath.abs(pattern.getEstimatedModuleSize() - average);
    }
  ...
  private FinderPattern[] selectBestPatterns() throws NotFoundException {
    ...
    if (startSize < 3) {
      // Couldn't find enough finder patterns
        System.err.println("not enough patterns " + startSize);
