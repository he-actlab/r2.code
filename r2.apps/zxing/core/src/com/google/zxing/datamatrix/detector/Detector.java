...
public final class Detector {
  ...
  private ResultPointsAndTransitions transitionsBetween(ResultPoint from, ResultPoint to) {
    ...
    int toX = (int) to.getX();
    int toY = (int) to.getY();
    boolean steep = ApproxMath.abs(toY - fromY) > ApproxMath.abs(toX - fromX);
    if (steep) {
      int temp = fromX;
      fromX = fromY;
      fromY = temp;
    ...
    int dx = ApproxMath.abs(toX - fromX);
    int dy = ApproxMath.abs(toY - fromY);
    int error = -dx >> 1;
    int ystep = fromY < toY ? 1 : -1;
    int xstep = fromX < toX ? 1 : -1;
    int transitions = 0;
    boolean inBlack = image.get(steep ? fromY : fromX, steep ? fromX : fromY);
    for (int x = fromX, y = fromY; x != toX; x += xstep) { /* EnerJ TODO */
