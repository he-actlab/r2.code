...
final class MultiFinderPatternFinder extends FinderPatternFinder {
    ...
    for (int i1 = 0; i1 < (size - 2); i1++) {
      ...
      for (int i2 = i1 + 1; i2 < (size - 1); i2++) {
        ...
        float vModSize12 = (p1.getEstimatedModuleSize() - p2.getEstimatedModuleSize()) /
            (ApproxMath.min(p1.getEstimatedModuleSize(), p2.getEstimatedModuleSize()));
        ...
        for (int i3 = i2 + 1; i3 < size; i3++) {
          ...
          float vModSize23 = (p2.getEstimatedModuleSize() - p3.getEstimatedModuleSize()) /
              (ApproxMath.min(p2.getEstimatedModuleSize(), p3.getEstimatedModuleSize()));
  ...
  public FinderPatternInfo[] findMulti(Hashtable hints) throws NotFoundException {
    ...
    for (int i = iSkip - 1; i < maxI; i += iSkip) {
        ...
        if (Loosen.loosen(image.get(j, i))) {
          ...
          if ((currentState & 1) == 0) { // Counting black pixels
            if (currentState == 4) { // A winner?
              if (Loosen.loosen(foundPatternCross(stateCount))) { // Yes
      ...
      if (Loosen.loosen(foundPatternCross(stateCount))) {
