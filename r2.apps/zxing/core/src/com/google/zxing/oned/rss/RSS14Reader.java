...
/* EnerJ unused
  ...
  private FinderPattern parseFoundFinderPattern(BitArray row, int rowNumber, boolean right, int[] startEnd)
    ...
    boolean firstIsBlack = Accept.accept(row.get(startEnd[0]));
    int firstElementStart = startEnd[0] - 1;
    // Locate element 1
    while (firstElementStart >= 0 && firstIsBlack ^ Accept.accept(row.get(firstElementStart))) {
      firstElementStart--;
    }
    firstElementStart++;
    ...
    int value = parseFinderValue(counters,(int [] []) FINDER_PATTERNS);
