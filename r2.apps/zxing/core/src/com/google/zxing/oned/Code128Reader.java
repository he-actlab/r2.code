...
public final class Code128Reader extends OneDReader {
  ...
  private static int[] findStartPattern(BitArray row) throws NotFoundException {
    ...
    int patternStart = rowOffset; // EnerJ TODO
      ...
      if (Loosen.loosen(cond)) {
        ...
        if (counterPosition == patternLength - 1) {
          ...
          if (bestMatch >= 0) {
            ...
            if (row.isRange(ApproxMath.max(0, patternStart - (i - patternStart) / 2), patternStart, false)) {
              ret = new int[]{patternStart, i, bestMatch};
            }
          }
  ...
  public Result decodeRow(int rowNumber, BitArray row, Hashtable hints)
    ...
    int codeSet;
    switch (Loosen.loosen(startCode)) {
      case CODE_START_A:
        codeSet = CODE_CODE_A;
        break;
      case CODE_START_B:
    ...
    int width = row.getSize();
    while (nextStart < width && Loosen.loosen(row.get(nextStart))) {
      nextStart++;
    }
    ...
    if (resultLength > 0 && Loosen.loosen(lastCharacterWasPrintable)) {
      if (Loosen.loosen(codeSet) == CODE_CODE_C) {
