...
if __name__ == '__main__':    
    ...
    if path == 'FFT' or path == 'SOR' or path == 'MonteCarlo' or path == 'SMM' or path == 'LU':
        results = collect_outputs(str(num) + 'scimark2', BMARKS[path], apronly, colonly, benchArg)
    else:
        results = collect_outputs(str(num) + path, BMARKS[path], apronly, colonly, benchArg)
    print '\n'.join(str(res) for res in results.itervalues())
    total.update(results)
    if path == 'FFT' or path == 'SOR' or path == 'MonteCarlo' or path == 'SMM' or path == 'LU':
        dump_json(total, str(num) + 'scimark2/' + JSON_OUT)
    else:
        dump_json(total, str(num) + path + '/' + JSON_OUT)
