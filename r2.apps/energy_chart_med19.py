...
#results = json_in()
results = json.load(open(str(num) + directory + '/results.json'))

# Constants.

SAVE_FPU_WIDTH = (0.32, 0.78, 0.83, 0.85)
SAVE_SRAM = (0.70, 0.80, 0.85, 0.90)
SAVE_DRAM = (0.17, 0.22, 0.23, 0.24)
SAVE_FUNCTIONAL_V = (0.12, 0.22, 0.27, 0.30)
...
# Header row.
table.append(['', 'TOTAL', 'DRAM', 'SRAM', 'Integer', 'FP'])

# Data rows.
for bmark, _ in BMLONGNAMES.iteritems():
    if not bmark in results: continue
    res = results[bmark]
    ...
    row += ('%.4f' % 1, dram_prop_base, sram_prop_base, 
    ...
    sum = '%.4f' % round(dram_prop_opt + sram_prop_opt + int_prop_opt + fp_prop_opt, 4) 
    row += (sum, dram_prop_opt, sram_prop_opt, 
