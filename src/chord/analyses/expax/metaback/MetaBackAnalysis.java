...
/**
 * The meta-backward analysis of EXPAX
 * 
 * @author xin
 * 
 */
					...
					if (!r.getType().isPrimitiveType()) { 
			            ...
			            for (Quad h: allocSites) {
			              if(SharedData.EXPAX_LOG)
			            	  System.out.println("*** EXPAX_LOG: [Backward] accept_all - allocation site = " + h.toString());
			        	  String tag = SharedData.getTag(Invoke.getMethod(invoke).getMethod());
			        	  if (!tag.equalsIgnoreCase(SharedData.allocToTagMap.get(h))){
			        		  if(SharedData.EXPAX_LOG)
			        			  System.out.println("*** EXPAX_LOG: [Backward] accept_all - Tag unmatched!");
			        		  continue;
			        	  }
			              ...
			              if (SharedData.EXPAX_LOG)
			        		System.out.println("*** EXPAX_LOG: [Backward] accept_all - accepted! array or object.field(" + hf.toString() + ")  (" + invoke.toString() + ")");
			              acceptedFs.add(hf);
			            }
					} else 
					...
					for(Map.Entry<Variable, Domain> predicate : c.getLiterals().entrySet()){
						...
						if(v instanceof VVariable){
							...
							if(!acceptedVs.contains(vv.getIdx()))
								...
								if (SharedData.EXPAX_LOG)
									System.out.println("*** EXPAX_LOG: [Backward] accept_all - REMOVED VVariable(" + vv.toString() + ")  (" + invoke.toString() + ")");
							}
						}
			            	...
			            	if(!acceptedFs.contains(new Pair<Integer,Integer>(hfv.getHIdx(),hfv.getFIdx())))
			            		...
			            		if (SharedData.EXPAX_LOG)
			            			System.out.println("*** EXPAX_LOG: [Backward] accept_all - REMOVED HFVariable(" + hfv.toString() + ")  (" + invoke.toString() + ")");
			            	}
			            }
			            	...
			            	if(!acceptedGs.contains(gv.getIdx()))
			            		...
			            		if (SharedData.EXPAX_LOG)
			            			System.out.println("*** EXPAX_LOG: [Backward] accept_all - REMOVED GVariable(" + gv.toString() + ")  (" + invoke.toString() + ")");
			            	}
			            }
						if(vv != null){
							if(d.equals(BoolDomain.T))
		...
		if(SharedData.isPreciseAllMethod(ivkedMeth)){
			...
			if (paramList.length() == 0){ // global
				...
				if (!r.getType().isPrimitiveType()) { 
					...
					for (Quad h: allocSites) {
						if(SharedData.EXPAX_LOG)
							System.out.println("*** EXPAX_LOG: [Backward] precise_all - allocation site = " + h.toString());
						String tag = SharedData.getTag(ivkedMeth);
			        	if (!tag.equalsIgnoreCase(SharedData.allocToTagMap.get(h))){
			        		if(SharedData.EXPAX_LOG)
			        			System.out.println("*** EXPAX_LOG: [Backward] precise_all - Tag unmatched!");
			        		continue;
			        	}
						int hidx = SharedData.domH.indexOf(h);
						...
						Pair<Integer,Integer> hf = new Pair<Integer,Integer>(hidx,fidx);
						if (SharedData.EXPAX_LOG)
							System.out.println("*** EXPAX_LOG: [Backward] precise_all - array or object.field(" + hf.toString() + ")  (" + q.toString() + ")");
						preciseFs.add(hf);
					}
				} else 
					throw new RuntimeException("Error! accept_all shouldn't have a primitive-type parameter");
					...
					if(v instanceof ErrVariable){
						if(d.equals(BoolDomain.T)){//err. err = err \/ tainted(v)}
							for(Integer vidx : preciseVs){
			            		...
			            		if (SharedData.EXPAX_LOG)
			            			System.out.println("*** EXPAX_LOG: [Backward] precise_all - ADDED VVariable(" + dnf2.toString() + ") (" + q.toString() + ")");
								dnf1 = dnf1.join(dnf2);
							}
							for(Pair<Integer,Integer> phfidx : preciseFs){
			            		...
			            		if (SharedData.EXPAX_LOG)
			            			System.out.println("*** EXPAX_LOG: [Backward] precise_all - ADDED HFVariable(" + dnf2.toString() + ") (" + q.toString() + ")");
				                dnf1 = dnf1.join(dnf2);
				            }
							for(Integer gidx : preciseGs){
								DNF dnf2 = new DNF(new ClauseSizeCMP(),new GVariable(gidx),BoolDomain.T);
			            		if (SharedData.EXPAX_LOG)
			            			System.out.println("*** EXPAX_LOG: [Backward] precise_all - ADDED GVariable(" + dnf2.toString() + ") (" + q.toString() + ")");
								dnf1 = dnf1.join(dnf2);
							}
						}else{//!err. !err = !err/\!tainted(v). Not sure if this is needed
							for(Integer vidx : preciseVs){
        	...
        	for (Clause c : iDNF.getClauses()) {
        		...
        		for(Map.Entry<Variable, Domain> predicate : c.getLiterals().entrySet()){
        			...
        			if(v instanceof HFVariable){
						...
						System.out.println("*** EXPAX_META: pair = " + pair.toString());
						if(hfv.getHIdx() == hidx && abs.approxStorage.contains(pair)){
							...
							System.out.println("*** EXPAX_META: stDNF = " + stDNF.toString());
							lDNF = lDNF.join(stDNF);
							System.out.println("*** EXPAX_META: lDNF = " + lDNF.toString());
						}
        			}
        			nc = nc.intersect(lDNF);
        		}
