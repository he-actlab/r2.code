...
def run(name, level, input):
    os.chdir(APP_ADDR)
    error = 0
    energy = 0
    failed_num = 0
    if level == 0:
        err_cmd = './collect_low.py ' + name + ' ' + input
        ...
        err_cmd = './collect_med.py ' + name + ' ' + input
        ...
        err_cmd = './collect_fd.py ' + name + ' ' + input
        ...
        err_cmd = './collect_high.py ' + name + ' ' + input
                ...
                for j in range(POPULATION_SIZE):
                    ...
                    if level == 0:
                        ...
                        for testInput in testInputs:
                            print "test input #%d ..." % index
                            index += 1
                            [error_low, energy_low] = run(name, level, testInput)
                            errorSum += error_low
                            energySum += energy_low
                        error_low = errorSum / float(len(testInputs))
                        energy_low = energySum / float(len(testInputs))
                        sg.updateInfo(LGeneration[jar_info.jar_name][j][0], error_low, energy_low)
                        ...
                        for testInput in testInputs:
                            print "test input #%d ..." % index
                            index += 1
                            [error_med, energy_med] = run(name, level, testInput)
                            errorSum += error_med
                            energySum += energy_med
                        error_med = errorSum / float(len(testInputs))
                        energy_med = energySum / float(len(testInputs))
                        sg.updateInfo(MGeneration[jar_info.jar_name][j][0], error_med, energy_med)
                        ...
                        for testInput in testInputs:
                            print "test input #%d ..." % index
                            index += 1
                            [error_fd, energy_fd] = run(name, level, testInput)
                            errorSum += error_fd
                            energySum += energy_fd
                        error_fd = errorSum / float(len(testInputs))
                        energy_fd = energySum / float(len(testInputs))
                        sg.updateInfo(FGeneration[jar_info.jar_name][j][0], error_fd, energy_fd)
                        FGeneration[jar_info.jar_name][j][2] = error_fd
                        FGeneration[jar_info.jar_name][j][3] = energy_med
                        ...
                        for testInput in testInputs:
                            print "test input #%d ..." % index
                            index += 1
                            [error_high, energy_high] = run(name, level, testInput)
                            errorSum += error_high
                            energySum += energy_high
                        error_high = errorSum / float(len(testInputs))
                        energy_high = energySum / float(len(testInputs))
                        sg.updateInfo(HGeneration[jar_info.jar_name][j][0], error_high, energy_high)
        ...
        for i in range(MAX_RUNS):
            ...
            for j in range(POPULATION_SIZE):
                ...
                if level == 0:
                    ...
                    error_low = errorSum / float(len(testInputs))
                    energy_low = energySum / float(len(testInputs))
                    sg.updateInfo(LGeneration[name][j][0], error_low, energy_low)
                    ...
                    error_med = errorSum / float(len(testInputs))
                    energy_med = energySum / float(len(testInputs))
                    sg.updateInfo(MGeneration[name][j][0], error_med, energy_med)
                    ...
                    error_fd = errorSum / float(len(testInputs))
                    energy_fd = energySum / float(len(testInputs))
                    sg.updateInfo(FGeneration[name][j][0], error_fd, energy_fd)
                    ...
                    error_high = errorSum / float(len(testInputs))
                    energy_high = energySum / float(len(testInputs))
                    sg.updateInfo(HGeneration[name][j][0], error_high, energy_high)
...
def findStatGuaranteeGene(bench, level):
    sg.sortOps()
...
def main():
    ...
    bench = sys.argv[1]
    level = sys.argv[2]
    print 'starting %s level %s...' % (bench, level)
    init(bench)
    geneticAlgorithm(bench, int(level), Inputs.BenchInputs['MonteCarlo'].trainInputs)
    findStatGuaranteeGene(bench, int(level))
    logging(bench)
