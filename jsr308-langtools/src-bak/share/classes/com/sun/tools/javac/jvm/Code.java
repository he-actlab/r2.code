...
public class Code {
	...
	private static final boolean EXPAX_TEST_CODE = false;
 ...
 ****************************************************************************/
    ...
    private void emitop(int op) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitop) bytecode offset = " + bytecodeOffset);
        if (pendingJumps != null) resolvePending();
        if (alive) {
    ...
    public void emitMultianewarray(int ndims, int type, Type arrayType) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitMultianewarray) bytecode offset = " + bytecodeOffset);
    ...
    public void emitNewarray(int elemcode, Type arrayType) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitNewarray) bytecode offset = " + bytecodeOffset);
        emitop(newarray);
        if (!alive) return;
        emit1(elemcode);
        state.pop(1); // count
    ...
    public void emitAnewarray(int od, Type arrayType) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitAnewarray) bytecode offset = " + bytecodeOffset);
    	emitop(anewarray);
        if (!alive) return;
        emit2(od);
        state.pop(1);
    ...
    public void emitInvokeinterface(int meth, Type mtype) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitInvokeinterface) bytecode offset = " + bytecodeOffset);
    ...
    public void emitInvokespecial(int meth, Type mtype) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitInvokespecial) bytecode offset = " + bytecodeOffset);    	
        int argsize = width(mtype.getParameterTypes());
    ...
    public void emitInvokestatic(int meth, Type mtype) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitInvokestatic) bytecode offset = " + bytecodeOffset);
        int argsize = width(mtype.getParameterTypes());
        emitop(invokestatic);
        if (!alive) return;
        emit2(meth);
    ...
    public void emitInvokevirtual(int meth, Type mtype) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitInvokevirtual) bytecode offset = " + bytecodeOffset);
        int argsize = width(mtype.getParameterTypes());
    ...
    public void emitInvokedynamic(int desc, Type mtype) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitInvokedynamic) bytecode offset = " + bytecodeOffset);
    ...
    public void emitop0(int op) {
    	if(EXPAX_TEST_CODE){
    		System.out.println("*** EXPAX_TEST_CODE: (emitop0) bytecode offset = " + bytecodeOffset);
    	}
        emitop(op);
        if (!alive) return;
        switch (op) {
        ...
        case saload:
            ...
            state.push(syms.intType);
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop0) iaload");
            break;
        case laload:
            state.pop(2);
            state.push(syms.longType);
        ...
        case sastore:
            state.pop(3);
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop0) iastore");
            break;
        case lastore:
        case dastore:
            state.pop(4);
    ...
    public void emitop1(int op, int od) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitop1) bytecode offset = " + bytecodeOffset);
        emitop(op);
        if (!alive) return;
        emit1(od);
        switch (op) {
    ...
    public void emitop1w(int op, int od) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitop1w) bytecode offset = " + bytecodeOffset);
        if (od > 0xFF) {
            emitop(wide);
            emitop(op);
            emit2(od);
    ...
    public void emitop2(int op, int od) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitop2) bytecode offset = " + bytecodeOffset);
        emitop(op);
        if (!alive) return;
        emit2(od);
        switch (op) {
        case getstatic:
            ...
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop2) getstatic");
            break;
        case putstatic:
            state.pop(((Symbol)(pool.pool[od])).erasure(types));
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop2) putstatic");
            break;
        case new_:
        ...
        case putfield:
            ...
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop2) putfield");
            break;
        case getfield:
            state.pop(1); // object ref
            state.push(((Symbol)(pool.pool[od])).erasure(types));
            if(EXPAX_TEST_CODE)
        		System.out.println("*** EXPAX_TEST_CODE: (emitop2) getfield");
            break;
        case checkcast: {
            state.pop(1); // object ref
    ...
    public void emitop4(int op, int od) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: (emitop4) bytecode offset = " + bytecodeOffset);
    	emitop(op);
        if (!alive) return;
        emit4(od);
        switch (op) {
 ...
 *************************************************************************/
    ...
    public int emitJump(int opcode) {
    	if(EXPAX_TEST_CODE)
    		System.out.println("*** EXPAX_TEST_CODE: emitJump");
        if (fatcode) {
