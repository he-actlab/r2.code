...
public class Random {
	...
	public final synchronized double nextDouble () {
		...
		k = m[i] - m[j]; // approx: load m[i] and m[j], sub, move to k
		// additional accept
		k = accept(k);
		if (k < 0) 
			k += m1; // approx: add, move
		m[j] = k; // approx: move to m[j]
		...
		if (haveRange) 
			return  left +  dm1 * k * width; // approx: load left + dm1 + width, 2 muls, 1 add
		else
			return dm1 * k; // approx: load dm1, mul
