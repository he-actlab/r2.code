...
final class GF256Poly {
  ...
  GF256Poly(GF256 field, int[] coefficients) {
    ...
    if (coefficientsLength > 1 && coefficients[0] == 0) {
      ...
      if (firstNonZero == coefficientsLength) {
        ...
        System.arraycopy(coefficients,	// approx: 24: MOVE_I T11, IConst: 0	// approx: 22: ARRAYLENGTH T10, T9
            firstNonZero,
            this.coefficients,
            0,
  ...
  GF256Poly addOrSubtract(GF256Poly other) {
    ...
    System.arraycopy(largerCoefficients, 0, sumDiff, 0, lengthDiff);	// approx: 27: MOVE_I T42, IConst: 0	// approx: 28: MOVE_I T43, IConst: 0
  ...
  public String toString() {
    StringBuffer result = new StringBuffer(8 * getDegree());	// approx: 4: MUL_I T4, IConst: 8, T3
    for (int degree = getDegree(); degree >= 0; degree--) {	// approx: 8: MOVE_I R15, T14	// approx: 49: ADD_I R41, R16, IConst: -1
      int coefficient = getCoefficient(degree);	// approx: 13: MOVE_I R18, T17
      if (coefficient != 0) {
        if (coefficient < 0) {
          result.append(" - ");
          coefficient = -coefficient;	// approx: 23: MOVE_I R22, T21
        } else {
          if (result.length() > 0) {
            result.append(" + ");
          }
        ...
        if (degree == 0 || coefficient != 1) {
          int alphaPower = field.log(coefficient);	// approx: 29: MOVE_I R10, T28
          if (alphaPower == 0) {
            result.append('1');	// approx: 39: MOVE_I T29, IConst: 49
          } else if (alphaPower == 1) {
            result.append('a');	// approx: 36: MOVE_I T31, IConst: 97
          } else {
            result.append("a^");
        ...
        if (degree != 0) {
          if (degree == 1) {
            result.append('x');	// approx: 47: MOVE_I T36, IConst: 120
          } else {
            result.append("x^");
