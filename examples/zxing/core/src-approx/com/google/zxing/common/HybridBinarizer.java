...
public final class HybridBinarizer extends GlobalHistogramBinarizer {
	...
	// is 8x8 pixels.
	// So this is the smallest dimension in each axis we can.
	...
	// a 5x5 grid
	// of the blocks around it. Also handles the corner cases, but will ignore
	// up to 7 pixels
	// on the right edge and 7 pixels at the bottom of the image if the overall
	// dimensions are not
	// multiples of eight. In practice, leaving those pixels white does not seem
	// to be a problem.
	private static void calculateThresholdForBlock(byte[] luminances,
			int subWidth, int subHeight, int stride, int[][] blackPoints,
			BitMatrix matrix) {
		for (int y = 0; y < subHeight; y++) {
			for (int x = 0; x < subWidth; x++) {
				...
				int average = sum / 25;
				threshold8x8Block(luminances, x << 3, y << 3, average, stride,
						matrix);
			}
		}
	}
	...
	private static int[][] calculateBlackPoints(byte[] luminances,
			int subWidth, int subHeight, int stride) {
		// original
		/*
