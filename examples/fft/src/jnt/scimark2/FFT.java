...
public class FFT {
	...
	public static void inverse (double data[]) {
		...
		double norm=1.0/aprN;	// approx: 9: DIV_D T7, DConst: 1.0, T14	// approx: 10: MOVE_D R8, T7
		for(int i=0; i<nd; i++)
	...
	protected static void transform_internal (double data[], int direction) {
		...
		for (int bit = 0, dual = 1; bit < logn; bit++, dual *= 2) {
			...
			double theta = 2.0 * direction * Math.PI / (2.0 * (double) dual);	// approx: 22: MOVE_D R12, T45	// approx: 21: DIV_D T45, T43, T44	// approx: 20: MUL_D T44, DConst: 2.0, T11	// approx: 17: MUL_D T42, DConst: 2.0, T41	// approx: 18: MUL_D T43, T42, DConst: 3.141592653589793
			...
			for (int b = 0; b < n; b += 2 * dual) {
				...
				data[j]   = data[i]   - wd_real; 	// approx: 111: ALOAD_D T63, R0, R54	// approx: 113: ASTORE_D T64, T62, R57	// approx: 112: SUB_D T64, T63, R59
				data[j+1] = data[i+1] - wd_imag; 	// approx: 117: ALOAD_D T68, R0, T66	// approx: 119: ASTORE_D T69, T67, T65	// approx: 118: SUB_D T69, T68, R30
				data[i]  += wd_real; 	// approx: 124: ASTORE_D T73, T71, T70	// approx: 123: ADD_D T73, T72, R59	// approx: 122: ALOAD_D T72, R0, R54
				data[i+1]+= wd_imag; 	// approx: 129: ADD_D T78, T77, R30	// approx: 128: ALOAD_D T77, R0, T74	// approx: 130: ASTORE_D T78, T76, T75
			}
			...
			for (int a = 1; a < dual; a++) {
				...
				{
					double tmp_real = w_real - s * w_imag - s2 * w_real; 	// approx: 43: MOVE_D R17, T89	// approx: 39: MUL_D T86, R13, R84	// approx: 40: SUB_D T87, R85, T86	// approx: 41: MUL_D T88, R15, R85	// approx: 42: SUB_D T89, T87, T88
					double tmp_imag = w_imag + s * w_real - s2 * w_imag; 	// approx: 48: MOVE_D R94, T93	// approx: 47: SUB_D T93, T91, T92	// approx: 44: MUL_D T90, R13, R85	// approx: 45: ADD_D T91, R84, T90	// approx: 46: MUL_D T92, R15, R84
				...
				for (int b = 0; b < n; b += 2 * dual) {
					...
					double wd_imag = w_real * z1_imag + w_imag * z1_real; 	// approx: 156: ADD_D T113, T111, T112	// approx: 157: MOVE_D R26, T113	// approx: 154: MUL_D T111, R95, R24	// approx: 155: MUL_D T112, R96, R23

					data[j]   = data[i]   - wd_real; 	// approx: 161: ASTORE_D T116, T114, R22	// approx: 160: SUB_D T116, T115, R25	// approx: 159: ALOAD_D T115, R0, R101
					data[j+1] = data[i+1] - wd_imag; 	// approx: 167: ASTORE_D T121, T119, T117	// approx: 166: SUB_D T121, T120, R26	// approx: 165: ALOAD_D T120, R0, T118
					data[i]  += wd_real; 	// approx: 172: ASTORE_D T125, T123, T122	// approx: 171: ADD_D T125, T124, R25	// approx: 170: ALOAD_D T124, R0, R101
					data[i+1]+= wd_imag; 	// approx: 176: ALOAD_D T129, R0, T126	// approx: 177: ADD_D T130, T129, R26	// approx: 178: ASTORE_D T130, T128, T127
				}
			}
		}
	}
	...
	protected static void bitreverse(double data[]) {
		...
		for (; i < nm1; i++) {
			...
			if (i < j) {
				double tmp_real    = data[ii]; 	// approx: 18: MOVE_D R10, T27	// approx: 17: ALOAD_D T27, R0, R6
				double tmp_imag    = data[ii+1]; 	// approx: 20: ALOAD_D T29, R0, T28	// approx: 21: MOVE_D R12, T29
				data[ii]   = data[jj]; 	// approx: 24: ASTORE_D T31, T30, R6	// approx: 23: ALOAD_D T31, R0, R7
