...
/* EnerJ unused
    @Approx int[] counters = dataCharacterCounters;
    counters[0] = 0;
    counters[1] = 0;
    counters[2] = 0;
    counters[3] = 0;
  ...
  private int[] findFinderPattern(BitArray row, int rowOffset, boolean rightFinderPattern)
    ...
    @Approx int[] counters = decodeFinderCounters;
    counters[0] = 0;
    counters[1] = 0;
    counters[2] = 0;
    counters[3] = 0;
    ...
    while (rowOffset < width) {
      ...
      if (Endorsements.endorse(rightFinderPattern == isWhite)) {
    ...
    for (int x = rowOffset; x < width; x++) {
      ...
      if (Endorsements.endorse(pixel ^ isWhite)) {
        ...
        if (counterPosition == 3) {
          if (Endorsements.endorse(isFinderPattern(counters))) {
  ...
  private FinderPattern parseFoundFinderPattern(BitArray row, int rowNumber, boolean right, int[] startEnd)
    ...
    boolean firstIsBlack = Endorsements.endorse(row.get(startEnd[0]));
    int firstElementStart = startEnd[0] - 1;
    // Locate element 1
    while (firstElementStart >= 0 && firstIsBlack ^ Endorsements.endorse(row.get(firstElementStart))) {
      firstElementStart--;
    }
    firstElementStart++;
